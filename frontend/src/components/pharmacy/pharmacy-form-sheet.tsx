...
import { useCreatePharmacy } from '@/hooks/use-pharmacies'
import type { CreatePharmacyInput, ApiError } from '@/types/pharmacy.types'
import { createPharmacySchema } from '@/schemas/pharmacy.schema'
import { cleanCNPJ, cleanCEP } from '@/lib/utils/brazilian-documents'
...
// Form validation schema - extend the base schema to include CNES field for UI
const pharmacyFormSchema = createPharmacySchema.extend({
...
interface PharmacyFormDrawerProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function PharmacyFormDrawer({
  open,
  onOpenChange,
}: PharmacyFormDrawerProps) {
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      ...
      const validationResult = pharmacyFormSchema.safeParse(value)
      ...
      const payload: CreatePharmacyInput = {
        cnpjNumber: cleanCNPJ(value.cnpjNumber),
      ...
      createPharmacy.mutate(payload, {
        onSuccess: () => {
          toast.success('Estabelecimento cadastrado com sucesso!', {
            description: `${payload.companyName} foi adicionado ao sistema.`,
          })
          form.reset()
          onOpenChange(false)
        },
          ...
          >
            ...
            <form.Field
              ...
              validators={{
                onChange: ({ value }) => {
                  const result =
                    pharmacyFormSchema.shape.cnpjNumber.safeParse(value)
                  if (!result.success) {
                    return result.error.errors[0]?.message ?? 'CNPJ inválido'
                  }
                  return undefined
                },
              }}
            ...
            <form.Field
              ...
              validators={{
                onChange: ({ value }) => {
                  const result =
                    pharmacyFormSchema.shape.companyName.safeParse(value)
                  if (!result.success) {
                    return (
                      result.error.errors[0]?.message ?? 'Razão social inválida'
                    )
                  }
                  return undefined
                },
              ...
              validators={{
                onChange: ({ value }) => {
                  ...
                  if (value && value.length > 0) {
                    const result =
                      pharmacyFormSchema.shape.address.safeParse(value)
                    if (!result.success) {
                      return (
                        result.error.errors[0]?.message ?? 'Endereço inválido'
                      )
                    }
                  }
                  return undefined
        ...
        <SheetFooter className="border-t p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full">
            ...
            <Button
              ...
              disabled={createPharmacy.isPending}
              onClick={() => void form.handleSubmit()}
            >
              {createPharmacy.isPending ? 'Salvando...' : 'Salvar'}
            </Button>
          </div>
        </SheetFooter>
      </SheetContent>
